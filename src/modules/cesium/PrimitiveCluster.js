...
function getScreenSpacePositions(
  ...
  for (let i = 0; i < length; ++i) {
    ...
    if (!defined(coord)) {
      console.error(`Failed to compute screen space position for item at index ${i}:`, item);
      continue;
    }
  ...
  // 打印 points 数组以进行调试
  console.log("points after getScreenSpacePositions", points);
}

// 在调用 createDeclutterCallback 之前，检查数据源
// console.log("labelCollection", this._labelCollection);
// console.log("billboardCollection", this._billboardCollection);
// console.log("pointCollection", this._pointCollection);

const declutterCallback = createDeclutterCallback(this);
declutterCallback();
...
function createDeclutterCallback(entityCluster) {
  return function (amount) {
    ...
    points.forEach((point, index) => {
      if (!point || !point.coord || typeof point.coord.x !== 'number' || typeof point.coord.y !== 'number') {
