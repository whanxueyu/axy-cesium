...
export function useSampledPositionProperty(
  ...
  startJulianDate = Cesium.JulianDate.addHours(
    startJulianDate,
    8,
  ...
  lines.forEach((line) => {
    const moveTime = Cesium.JulianDate.addSeconds(
      startJulianDate,
      line.time,
      new Cesium.JulianDate(),
    );
...
export function cartesian2ToWGS84(
  viewer: Cesium.Viewer,
  ...
  if (Cesium.defined(worldPosition)) {
    // 转换为经纬度
    const cartographic =
      Cesium.Ellipsoid.WGS84.cartesianToCartographic(worldPosition);
...
export function translateTiles(
  ...
  Cesium.Matrix4.setTranslation(
    ...
    new Cesium.Cartesian3(options.x, options.y, options.z),
    m,
  ); //构造平移矩阵
  // @ts-ignore
 ...
 */
export const isTerrainEnabled = (viewer: Cesium.Viewer): boolean => {
 ...
 * @returns {Promise<number>} 地形高度（米），如果失败则返回0
 */
export const sampleTerrainHeight = async (
  viewer: Cesium.Viewer,
  longitude: number,
  latitude: number,
): Promise<number> => {
  if (!viewer || !isTerrainEnabled(viewer)) return 0;

  const provider = viewer.terrainProvider || viewer.scene.terrainProvider;

  try {
    const updatedPositions = await Cesium.sampleTerrainMostDetailed(provider, [
      Cesium.Cartographic.fromDegrees(longitude, latitude),
    ]);

    return updatedPositions?.[0]?.height ?? 0;
  } catch (error) {
    console.warn("地形采样失败, 默认返回0:", error);
    return 0;
  }
};
