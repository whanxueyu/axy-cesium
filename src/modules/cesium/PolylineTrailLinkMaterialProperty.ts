...
class PolylineTrailLinkMaterialProperty {
  ...
  private readonly repeatNum: number;

  constructor(image: string | undefined, color: Color = Color.WHITE, duration: number = 1000, repeatNum: number = 3) {
    console.log('PolylineTrailLinkMaterialProperty', ...arguments);
    ...
    (Material as any)._materialCache.addMaterial('PolylineTrailLink', {
      fabric: {
        ...
        czm_material czm_getMaterial(czm_materialInput materialInput) {
          ...
          vec4 sampledColor = texture(image, vec2(fract(repeatNum*st.s - time), st.t));
  ...
  // equals(other: PolylineTrailLinkMaterialProperty): boolean | undefined {
  ...
  equals(other: PolylineTrailLinkMaterialProperty): boolean {
    ...
    // 使用 defined 确保 _color 存在且非 undefined
    if (!defined(this._color) || !defined(other._color)) {
      return false;
    }
