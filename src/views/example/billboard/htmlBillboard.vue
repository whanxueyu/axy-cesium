...
import { sampleTerrainHeight } from "@/modules/cesium/tools";
...
const addBillboard = async () => {
  for (const item of htmlOverList) {
    let h = item.position[2];
    // 如果高度为0且启用地形，则采样高度
    if (h === 0) {
      h = await sampleTerrainHeight(viewer, item.position[0], item.position[1]);
      console.log(h, "5555");
    }
    ...
    htmlOverlay.style.pointerEvents = item.pointEvents ?? "auto";
