<template>
    <div :class="['menubox box1', showPanel ? '' : 'hide']">
        ...
        <div class="el-tabs">
            <!-- {{ viewer.clock.currentTime.toString() }} -->
            <div class="itemList">
...
import DynamicSkybox from '@/modules/cesium/dynamicSkybox.ts';

var viewer: Cesium.Viewer;
let dynamicSkybox: DynamicSkybox | undefined;
...
const handleMapLoaded = (Viewer: Cesium.Viewer) => {
    viewer = Viewer;
    // 初始化 DynamicSkybox
    dynamicSkybox = new DynamicSkybox({ sources: groundSkyboxList[0].source });
    dynamicSkybox.setSkyBox(viewer);
    viewer.clock.multiplier = 1440;
    viewer.clock.shouldAnimate = true;
};

const selectSkybox = (skybox: skyboxListType) => {
    currentSkyBox.value = skybox;
    if (dynamicSkybox && skybox.source) {
        dynamicSkybox.sources = skybox.source;
    }
};
